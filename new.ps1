...
Write-Info "Valid until: $($cert.NotAfter.ToString('yyyy-MM-dd'))"
...
# Export CER (public key only, for upload reference)
...
# Get base64 cert value for app registration
...
║  Cert Expires:    $($cert.NotAfter.ToString('yyyy-MM-dd'))
║                                                              ║
╚══════════════════════════════════════════════════════════════╝
...
Write-Info "The certificate is still in Cert:\CurrentUser\My\$($cert.Thumbprint)"
Write-Info "You can remove it after copying the PFX to the build agent:"
Write-Info "  Remove-Item -Path 'Cert:\CurrentUser\My\$($cert.Thumbprint)'"
#endregion
